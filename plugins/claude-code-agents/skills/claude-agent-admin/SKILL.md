...
3. 使用 `list_runners` 确认目标 CLI 的安装状态和能力声明。
4. 使用 `run_agent(..., dryRun=true)`，检查最终 CLI 参数、原生 Agent 加载或角色提示词注入和委派预览。
5. 确认目标 Runner 的 `--version` 与本地登录状态。
5. 检查 Codex 中插件 MCP server 是否启用，以及 `run_agent` 的工具审批策略。
6. 实际调用失败时，读取 `stderr`；不要输出或复制完整环境。

顺序任务默认使用 `background=false`，MCP 服务端等待 Agent 完成后只返回一次紧凑结果；后台任务使用 `background=true` 后调用一次 `job_wait`，不要让 Codex 通过 `job_status` 循环轮询。`job_status` 只读状态，后台 Worker 根据活动自行续租；Worker 空闲超时或 MCP 所有者断开时任务才会终止。
