...
const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        agent: { type: 'string', description: 'Required role id or alias, e.g. backend-engineer or 后端工程师.' },
        runner: { type: 'string', enum: ['claude', 'codex', 'grok', 'agy'], description: 'Deprecated optional field. Omit it. Execution channel is configuration-driven; only a value equal to the role configured default runner is accepted, and any mismatch is rejected. Switch channels by updating <ROLE>_DEFAULT_RUNNER.' },
        ...
        model: { type: 'string', description: 'One-run override; normally resolved from layered configuration (env files, SQLite, process env).' },
        ...
        timeoutMs: { type: 'integer', minimum: 1000, description: 'Optional Runner execution timeout override. Omit to use the configured role timeout.' },
        allowShorterTimeout: { type: 'boolean', default: false, description: 'Permit timeoutMs to shorten the configured role timeout. Use only when the user explicitly requests a shorter execution limit.' },
